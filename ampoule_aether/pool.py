...
from ampoule_aether import commands, main
...
class ProcessPool(object):
    ...
    def __init__(self, ampChild=None, ampParent=None, min=5, max=20,
        ...
        if ampChild is None:
            from ampoule_aether.child import AMPChild
            self.ampChild = AMPChild
        self.min = min
        self.max = max
        self.name = name
    ...
    def dumpStats(self):
        print("ProcessPool stats:")
        print('\tworkers: %s' % len(self.processes))
        print('\ttimeout: %s' % (self.timeout))
        print('\tparent: %r' % (self.ampParent,))
        print('\tchild: %r' % (self.ampChild,))
        print('\tmax idle: %r' % (self.maxIdle,))
        print('\trecycle after: %r' % (self.recycleAfter,))
        print('\tProcessStarter:')
        print('\t\t%r' % (self.starter,))
